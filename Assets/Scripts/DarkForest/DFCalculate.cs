...
public class DFCalculate : MonoBehaviour
{
  ...
  {
    time = 0;
    GameObject.Find("UI/Message").GetComponent<Text>().text = "初始化完毕";
    for (int i = 0; i < stars.Count; i++)
      if (stars[i].life)
        Destroy(GameObject.Find("Stars/Star" + i.ToString()).GetComponent<Transform>().gameObject);
    stars.Clear(); deathcnt = 0;
    ...
    for (int i = 0; i < DFGlobal.startcnt; i++)
      spawnstar();
  }

  static void ReadDFGlobal()
  {
    if (File.Exists(Application.dataPath + @"\start_up_parameter.txt"))
    {
      ...
      DFGlobal.startcnt = int.Parse(sr.ReadLine());
      DFGlobal.startscore = int.Parse(sr.ReadLine());
      DFGlobal.travel_speed = double.Parse(sr.ReadLine());
      DFGlobal.develop = int.Parse(sr.ReadLine());
      DFGlobal.cooperation = int.Parse(sr.ReadLine());
      DFGlobal.attack = int.Parse(sr.ReadLine());
      DFGlobal.allowspawn = bool.Parse(sr.ReadLine());
      DFGlobal.spawnprobability = int.Parse(sr.ReadLine());
      DFGlobal.cooldowntime = int.Parse(sr.ReadLine());
      DFGlobal.rangex = double.Parse(sr.ReadLine());
      DFGlobal.rangey = double.Parse(sr.ReadLine());
      DFGlobal.rangez = double.Parse(sr.ReadLine());
      DFGlobal.defensetimes = int.Parse(sr.ReadLine());
      DFGlobal.peace = int.Parse(sr.ReadLine());
      DFGlobal.middle = int.Parse(sr.ReadLine());
      DFGlobal.attacks = int.Parse(sr.ReadLine());
      DFGlobal.allowtechboom = bool.Parse(sr.ReadLine());
      DFGlobal.techboommax = int.Parse(sr.ReadLine());
      DFGlobal.techboom_addon = int.Parse(sr.ReadLine());
      DFGlobal.techboom_probability = int.Parse(sr.ReadLine());
      DFGlobal.allow2d = bool.Parse(sr.ReadLine());
      DFGlobal.score2d = int.Parse(sr.ReadLine());
      DFGlobal.speed2d = double.Parse(sr.ReadLine());
      DFGlobal.cooldown2d = int.Parse(sr.ReadLine());
      DFGlobal.allow_attack_help = bool.Parse(sr.ReadLine());
      sr.Close();
  ...
  bool isattack(int start, int target)
  {
    ...
    {
      if (stars[target].type == 1) if (DFGlobal.allow_attack_help) return false; else return true;
      return false;
    }
  ...
  void spawnstar()
  {
    int num = rd.Next() % (DFGlobal.peace + DFGlobal.middle + DFGlobal.attacks), asd;
    if (num < DFGlobal.peace) asd = -1;
    else if (DFGlobal.peace <= num && num < DFGlobal.peace + DFGlobal.middle) asd = 0;
    else asd = 1;

    stars.Add(new Stars(stars.Count + 1, (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangex), (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangey),
        (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangez), asd, rd.Next() % 2 == 1, time));
  ...
  void Update()
  {
    ...
    GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text = "";
    time += 1;//加一年
    GameObject.Find("UI/Time").GetComponent<Text>().text = time.ToString() + "年";

    //生成新文明
    if (DFGlobal.allowspawn)
      if (rd.Next() % DFGlobal.spawnprobability == 0)
        spawnstar();
    ...
    for (int i = 0; i < bfs.Count; i++)//二向箔计算
    {
      ...
      bfs[i].distance += DFGlobal.speed2d;
      ...
      if (bfs[i].distance >= bfs[i].total)//到达目的地
      {
        ...
        {
          stars[bfs[i].target].x = (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangex);
          stars[bfs[i].target].y = (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangey);
          stars[bfs[i].target].z = (float)((rd.NextDouble() - 0.5) * 2 * DFGlobal.rangez);
    ...
    for (int i = 0; i < stars.Count; i++)
    {
      ...
      stars[i].score += DFGlobal.develop + stars[i].helpcnt * DFGlobal.cooperation + stars[i].techboomcnt * DFGlobal.techboom_addon;//加分
      if (DFGlobal.allowtechboom)//技术爆炸代码
      {
        if (stars[i].techboomcnt < DFGlobal.techboommax)
          if (rd.Next() % DFGlobal.techboom_probability == 0)
          {
      ...
      if (stars[i].isout)
      {
        if (stars[i].havetarget)//飞船计算代码
        {
          ...
          else if (stars[i].ship.stats == 0)//飞行状态
          {
            ...
            stars[i].ship.distance += DFGlobal.travel_speed;
          ...
          else
          {
            ...
            else if (stars[i].ship.type == 1)//打起来了。。。
            {
              ...
              {
                stars[stars[i].ship.target].score += DFGlobal.attack;
                stars[i].ship.defense += DFGlobal.attack;
              }
            }
          }
        }
        else//新建飞船实体
        {
          ...
          if (stars[i].score < DFGlobal.cooldowntime) continue;//发展不足，同志仍需努力(～￣▽￣)～

          int target = rd.Next() % stars.Count;

          if (DFGlobal.allow2d && stars[i].score >= DFGlobal.score2d)//二向箔启用判断
            if (stars[target].score > stars[i].score && isattack(i, target) && time - stars[i].time2d >= DFGlobal.cooldown2d / 2)//二向箔，条件：目标文明得分大于发出者得分一半
            {
              stars[i].time2d = time;
          ...
          Ships ship = new Ships(i, target, stars[i].score / DFGlobal.defensetimes, stars[i], stars[target]);
    ...
    GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text = "文明总数：" + (stars.Count - deathcnt).ToString()
    ...
    if (sortscore)
    {
      ...
      for (int i = 0; i < stars.Count; i++)
        if (temp[i].life)
        {
          switch (temp[i].type)
          {
            case 1: { GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += "<color=#FF0000>"; break; }
            case -1: { GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += "<color=#00FF00>"; break; }
            case 0: { GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += "<color=#FFFF00>"; break; }
            default: { GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += "<color=#FFFFFF>"; break; }
          }
          GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += (temp[i].score > temp[i].scorelast ? "↑" : "↓") + temp[i].num.ToString() + "号文明得分：" + temp[i].score.ToString()
      ...
      for (int i = 0; i < stars.Count; i++)
        if (stars[i].life)
        {
          ...
          GameObject.Find("UI/ScoreBoard").GetComponent<Text>().text += (stars[i].score > stars[i].scorelast ? "↑" : "↓") + stars[i].num.ToString() + "号文明得分：" + stars[i].score.ToString()
